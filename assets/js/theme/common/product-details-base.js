...
export default class ProductDetailsBase {
    ...
    getViewModel($scope) {
        return {
            $priceWithTax: $('.productView-price [data-product-price-with-tax]', $scope),
            $priceWithoutTax: $('.productView-price [data-product-price-without-tax]', $scope),
            rrpWithTax: {
                $div: $('.productView-price .rrp-price--withTax', $scope),
                $span: $('.productView-price [data-product-rrp-with-tax]', $scope),
            },
            rrpWithoutTax: {
                $div: $('.productView-price .rrp-price--withoutTax', $scope),
                $span: $('.productView-price [data-product-rrp-price-without-tax]', $scope),
            },
            nonSaleWithTax: {
                $div: $('.productView-price .non-sale-price--withTax', $scope),
                $span: $('.productView-price [data-product-non-sale-price-with-tax]', $scope),
            },
            nonSaleWithoutTax: {
                $div: $('.productView-price .non-sale-price--withoutTax', $scope),
                $span: $('.productView-price [data-product-non-sale-price-without-tax]', $scope),
            },
            priceSaved: {
                $div: $('.productView-price .price-section--saving', $scope),
                $span: $('.productView-price [data-product-price-saved]', $scope),
            },
            priceNowLabel: {
                $span: $('.productView-price .price-now-label', $scope),
            },
            priceLabel: {
                $span: $('.productView-price .price-label', $scope),
            },
    ...
    updateView(data, content = null) {
        ...
        if (typeof data.stock === 'number') {
            if((data.stock <= parseInt(this.context.themeSettings.halo_stock_level_limit)) && (data.stock > 0)) {
                viewModel.$stockLeftWrapper.removeClass('u-hiddenVisually').addClass('halo-fadeIn');
    ...
    hotStock(inventoryQuantity) {
        ...
        const stockCurrent = productView.querySelector('[data-current-inventory]');
        if(hotStock) {
            let maxStock = parseFloat(hotStock.dataset.hotStock);
